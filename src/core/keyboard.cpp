...
bool Keyboard::isConnected() { return isConnected_; }
...
void Keyboard::disconnect() { isConnected_ = false; }
...
void Keyboard::playMacro(std::string macroPath, VirtualInput *virtInput) {
  ...
  if (!xmlDoc.ErrorID()) {
    ...
    for (tinyxml2::XMLElement *child = root->FirstChildElement(); child;
      ...
      } else if (child->Name() == std::string("SystemEvent")) {
        /*
         * This can only be written into a macro file.
         * Also yes, we are really doing this, fight me bitch
         */
        system(std::format("setsid -f {}", child->GetText()).c_str());
      }
    }
  }
}
 ...
 * capturing delays.
 */
void Keyboard::recordMacro(std::string path, Led *ledRecord,
                           const int keyRecord) {
  struct timeval prev;
  ...
  while (isRecordMode) {
    ...
    if (inev.type == EV_KEY && inev.value != 2) {
      ...
      if (prev.tv_usec && config_->lookup("capture_delays")) {
        auto diff = (inev.time.tv_usec + 1000000 * inev.time.tv_sec) -
                    (prev.tv_usec + 1000000 * prev.tv_sec);
...
Keyboard::Keyboard(struct Device *device, sidewinderd::DevNode *devNode,
                   libconfig::Config *config, Process *process)
    : hid_{&fd_} {
  config_ = config;
  process_ = process;
  device_ = *device;
