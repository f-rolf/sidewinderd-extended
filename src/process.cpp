...
bool Process::isActive() { return isActive_; }

void Process::setActive(bool isActive) { isActive_ = isActive; }
...
void Process::setName(std::string name) { name_ = name; }
...
int Process::createPid(std::string pidPath) {
  pidPath_ = pidPath;
  pidFd_ = open(pidPath_.c_str(), O_CREAT | O_RDWR,
  ...
  if (flock(pidFd_, LOCK_EX | LOCK_NB) < 0) {
    std::cerr << "Could not lock PID file, another instance is already running."
              << std::endl;
    close(pidFd_);
...
void Process::privilege() { seteuid(0); }
...
std::string Process::getVersion() { return version; }

void Process::sigHandler(int sig) {
  std::cerr << std::endl << "Signal received: " << sig << std::endl;
